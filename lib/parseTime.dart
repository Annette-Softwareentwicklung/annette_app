...
String parseTimeToUserOutput (String s, [DateTime? pComparisonTime]) {
  ...
  DateTime comparisonTime = (pComparisonTime != null) ? pComparisonTime : DateTime.now();

  if(temp.year == comparisonTime.year && temp.month == comparisonTime.month && temp.day == comparisonTime.day) {
    weekday = 'Heute';
  } else if(temp.year == comparisonTime.year && temp.month == comparisonTime.month && temp.day == comparisonTime.add(Duration(days: 1)).day) {
    weekday = 'Morgen';
  } else if(temp.year == comparisonTime.year && temp.month == comparisonTime.month && temp.day == comparisonTime.add(Duration(days: 2)).day) {
    weekday = 'Übermorgen';
  } else if(temp.year == comparisonTime.year && temp.month == comparisonTime.month && temp.day == comparisonTime.subtract(Duration(days: 1)).day) {
    weekday = 'Gestern';
  } else if(temp.year == comparisonTime.year && temp.month == comparisonTime.month && temp.day == comparisonTime.subtract(Duration(days: 2)).day) {
    weekday = 'Vorgestern';
  }
